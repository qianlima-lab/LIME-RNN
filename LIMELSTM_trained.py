...
def main(_):
	...
	with tf.Session(config=gpu_config) as session:  
		...
		with tf.variable_scope("model", reuse=False):
			mtest = LIMELSTM(is_training=False,config=eval_config,FLAGS=FLAGS)

		saver = tf.train.Saver()
		model_name = "./Models/saved.ckpt"
		saver.restore(session, model_name)
		print "\nLoad model completed"
		_,prediction=run_epoch(session, mtest,test_data,tf.no_op()) 
		RMSE=reader.count_RMSE_matrix(raw,fil,np.reshape(np.array(prediction),[-1,FLAGS.embedding_size]),FLAGS.embedding_size,eval_config.num_steps)
		print 'test imputation RMSE:', RMSE		
